...
const dog = () =>
  fetch("dogs").then(({ data }) =>
    data.dogs.reduce((previousValue, currentValue) =>
      previousValue.naughty > currentValue.naughty
        ? previousValue
        : currentValue
    )
  );
...
const joke = () => {
  ...
  return fetch("jokes", "question")
    .then((response) => {
      questionAnswer.question = response.joke;
      return fetch("jokes");
    })
    .then((response) => {
      questionAnswer.answer = response.answer;
